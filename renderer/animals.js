...
(function () {
  ...
  function draw(ctx, _animal, state, now) {
    ...
    // keyboard
    const kbX = cx - 46, kbW = 92, kbY = deskY + 8, kbH = 22
    ...
    if (state.name) {
      ...
      ctx.lineWidth = 3; ctx.strokeStyle = 'rgba(90,60,30,0.5)'
      ctx.strokeText(state.name, cx, deskY + BAR_VIS - 9)
      ctx.fillStyle = '#fff'; ctx.fillText(state.name, cx, deskY + BAR_VIS - 9)
